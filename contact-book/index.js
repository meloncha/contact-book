...
app.put('/contacts/:id', (req, res) => {
    ...
    // Model.findOneAndUpdate 는 DB에서 해당 model의 document를 하나 찾아 그 data를 수정하는 함수입니다.
    // 첫번째 parameter로 찾을 조건을 object로 입력하고 두번째 parameter로 update할 정보를 object로 입력 data를 찾은 후
    // callback함수를 호출합니다. 이때 callback함수로 넘겨지는 값은 수정되기 전의 값입니다. 만약 업데이트 된 후의 값을 보고 싶다면
    // 콜백 함수 전에 parameter 로 { new : true }를 넣어주면 됩니다.
    Contact.findOneAndUpdate({ _id: req.params.id }, req.body, (err, contact) => {
        if (err) return res.json(err);
        res.redirect(`/contacts/${req.params.id}`);
        // Data 수정 후 '/contacts' + req.params.id 로 redirect
    });
});
// Contacts - destroy
app.delete('/contacts/:id', (req, res) => {
    ...
    // 첫번째 parameter로 찾을 조건을 object로 입력, data를 찾은 후 callback 함수를 호출
    Contact.deleteOne({ _id: req.params.id }, (err) => {
        if (err) return res.json(err);
        res.redirect('/contacts');
        // Data 삭제 후 '/contacts' 로 redirect
    });
});
